...
using System.Globalization; // 👈 make sure this is at the top
        ...
        public async Task<IActionResult> List(SearchProductViewModel model)
        {
            ...
            try
            {
                ...
                if (serviceCountResult.IsSucceeded && serviceListResult.IsSucceeded)
                {
                    ...
                    {
                        if (decimal.TryParse(item.Price, out decimal parsedPrice))
                        {
                            item.Price = "₹" + parsedPrice.ToString("N2", new CultureInfo("en-IN"));
                        }
                    }
                    jsonDataTableModel.aaData = listVM;
                    jsonDataTableModel.iTotalDisplayRecords = serviceCountResult.TransactionResult;
                    ...
                    */
                    ...
                    // ✅ Format price as ₹ using Indian number formatting
                    foreach (var item in listVM)
                    {
                        if (decimal.TryParse(item.Price, out decimal parsedPrice))
                        {
